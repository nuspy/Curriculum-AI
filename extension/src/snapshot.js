// Snapshot indicizzato della pagina (gemello JS di src/autojob/browser/snapshot_js.py).
// Iniettato via chrome.scripting.executeScript({func: buildSnapshot}); deve essere self-contained.
export function buildSnapshot() {
  ...
  const labelText = (el) => {
    ...
    if (el.id) {
      try { const lab = el.ownerDocument.querySelector('label[for="' + CSS.escape(el.id) + '"]'); if (lab && lab.textContent.trim()) return lab.textContent.trim(); } catch (e) {}
    }
  ...
  const collect = (root, framePath) => {
    ...
    for (const el of nodes) {
      ...
      const tag = el.tagName.toLowerCase();
      let options = null;
      if (tag === "select") options = Array.from(el.options).map((o) => ({ value: o.value, label: o.label || o.textContent, selected: o.selected }));
  ...
  for (const f of Array.from(document.querySelectorAll("form"))) {
    ...
    forms.push({ group_id: gid, action: f.getAttribute("action"), method: (f.getAttribute("method") || "get"), field_indexes: out.filter((n) => n.group_id === gid).map((n) => n.index) });
  }
